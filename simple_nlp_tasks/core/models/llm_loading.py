...
def load_rwkv(model_variant: str) -> Tuple[PreTrainedModel, PreTrainedTokenizer]:
    ...
    tokenizer = AutoTokenizer.from_pretrained(f"RWKV/rwkv-4-{model_variant}-pile",
                                              trust_remote_code=True, padding_side='left')
    # model = RwkvForCausalLM.from_pretrained(f"sgugger/rwkv-{model_variant}-pile")
    # tokenizer = AutoTokenizer.from_pretrained(f"sgugger/rwkv-{model_variant}-pile", padding_side='left')
    tokenizer.pad_token = tokenizer.eos_token
    return model, tokenizer
...
MODEL_PATHS = {
    ...
    "rwkv": {
        "430m": "sgugger/rwkv-430M-pile",
        "7B": "sgugger/rwkv-7b-pile"
    },
    "gla": {
        "1B": "bailin28/gla-1B-100B",
    },
    # "llama": {
